...
$new_domain_name = $module.Params.new_domain_name
$new_domain_netbios_name = $module.Params.new_domain_netbios_name
$parent_domain_name = $module.Params.parent_domain_name
...
$no_global_catalog = $module.Params.no_global_catalog
$critial_replication_only = $module.Params.critical_replication_only
$replication_source_dc = $module.Params.replication_source_dc
...
if (-not $forest) {
	...
	$Credentials = New-Object -TypeName "System.Management.Automation.PSCredential" -ArgumentList $credential_username,$credential_password
	$installParams.Credential = (Get-Credential -Credential $Credentials)
    }

    if ($domain_type) {
	$installParams.DomainType = $domain_type
    }

    if ($new_domain_name) {
	$installParams.NewDomainName = $new_domain_name
    }

    if ($new_domain_netbios_name) {
	$installParams.NewDomainNetbiosName = $new_domain_netbios_name
    }

    if ($parent_domain_name) {
	$installParams.ParentDomainName = $parent_domain_name
    }

    if ($site_name) {
	$installParams.SiteName = $site_name
    }

    if ($no_global_catalog) {
	$installParams.NoGlobalCatalog = $no_global_catalog
    }

    if ($critical_replication_only) {
	$installParams.CriticalReplicationOnly = $critical_replication_only
    }
    if ($replication_source_dc) {
	$installParams.ReplicationSourceDC = $replication_source_dc
    }
